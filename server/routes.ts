...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/credits", async (req, res) => {
    try {
      ...
      // Verify active membership status and sync credits accordingly
            ...
            if (validMemberships.length > 0) {
              ...
              if (planId === "plan_mndBT74OUdiNB" || planId === "plan_ZLLdnJssCPIpY") {
                ...
                const renewalPeriodStart = activeMembership.renewal_period_start 
                  ? new Date(activeMembership.renewal_period_start)
                  : activeMembership.created_at
                    ? new Date(activeMembership.created_at)
                    : new Date();
                
                const currentPeriodStartMs = renewalPeriodStart.getTime();

                // Get stored user details directly from MongoDB to check last reset
                const userDoc = await UserModel.findOne({ id: userId });
                const storedPeriodStartMs = userDoc?.currentPeriodStart 
                  ? new Date(userDoc.currentPeriodStart).getTime()
                  : 0;

                // If new billing period or newly subscribed, reset credits to plan limit
                if (storedPeriodStartMs !== currentPeriodStartMs || userDoc?.subscriptionPlan !== planId) {
                  console.log(`[Credits] Resetting credits for user ${userId} to ${planLimit} for billing cycle starting ${renewalPeriodStart.toISOString()}`);
                  await UserModel.findOneAndUpdate(
                    { id: userId },
                    {
                      $set: {
                        ...
                        currentPeriodStart: renewalPeriodStart,
                        updatedAt: new Date()
                      }
                    }
                  );
              ...
              // No valid active memberships found. Revoke subscription if previously recorded.
              const userDoc = await UserModel.findOne({ id: userId });
              if (userDoc?.subscriptionPlan) {
                console.log(`[Credits] Revoking subscription for user ${userId} (no active membership found)`);
                await UserModel.findOneAndUpdate(
                  { id: userId },
                  {
                    $set: {
  ...
  app.post("/api/credits/checkout-config", async (req, res) => {
    try {
      ...
      const allowedPlans = ["plan_mndBT74OUdiNB", "plan_ZLLdnJssCPIpY"];
      if (!planId || !allowedPlans.includes(planId)) {
        return res.status(400).json({ error: "Invalid or unsupported plan ID selected" });
      }
  ...
  app.post("/api/credits/process-payment", async (req, res) => {
    try {
      ...
      try {
        ...
        console.log(`[Process Payment] ✅ Granted ${planLimit} credits to customer: ${customerEmail || customerUserId} (Plan: ${planId})`);
      } catch (error) {
        console.error(`[Process Payment] ❌ Failed to grant subscription access:`, error);
    ...
    try {
      ...
      try {
        ...
        for await (const membership of whopSdk.memberships.list({
          ...
          // Only include Cursai or CryptoMind AI product memberships
          const productTitle = membership.product?.title || "";
          const isCursProduct = productTitle.toLowerCase().includes("curs") || productTitle.toLowerCase().includes("cryptomind");

          if (!membership.canceled_at && isCursProduct) {
            allMemberships.push(membership);
          }
        }

        console.log(`[Whop] Found ${allMemberships.length} total active Cursai memberships in owner's company`);
  ...
  app.get("/api/admin/stored-members", async (req, res) => {
    try {
      ...
      // Filter to only show Cursai or CryptoMind AI product members
      const members = allMembers.filter(m =>
        m.productTitle?.toLowerCase().includes("curs") || m.productTitle?.toLowerCase().includes("cryptomind")
      );
  ...
  app.post("/api/webhooks/payment", async (req, res) => {
    try {
      ...
      if (eventType === "payment.succeeded") {
        ...
        // ================================================================
        // GRANT SUBSCRIPTION CREDITS TO CUSTOMER (Whop Best Practice)
        // ================================================================
        try {
          const planId = payment.plan?.id || payment.membership?.plan?.id || "plan_mndBT74OUdiNB";
          const planLimit = planId === "plan_mndBT74OUdiNB" ? 50 : 20;
          ...
          console.log(`[Webhook] ✅ Granted ${planLimit} credits to customer: ${customerEmail || customerUserId} (Plan: ${planId})`);
        } catch (error) {
          console.error(`[Webhook] ❌ Failed to grant subscription access to customer:`, error);
        ...
        if (incomingPaymentAmount !== SUBSCRIPTION_PRICE_CENTS) {
          console.warn(`[Webhook] ⚠️  Incoming payment amount ($${incomingPaymentAmount / 100}) doesn't match plan price $${SUBSCRIPTION_PRICE_CENTS / 100}`);
          console.warn(`[Webhook] ENFORCING fixed price - commission will be $${COMMISSION_CENTS / 100} regardless of incoming amount`);
        }
  ...
  function handleNewSession(ws: WebSocket, history: Array<any>) {
    ...
    const welcomeMsg: ServerMessage = {
      type: "bot_message",
      content: "Welcome to Cursai! I provide real-time crypto and forex predictions powered by AI analysis. Simply pick a trading pair below to get started.",
    };
