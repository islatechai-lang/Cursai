            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3 p-3 rounded-lg bg-card border border-border/50">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {credits.subscriptionPlan === "plan_mndBT74OUdiNB" 
                      ? "Basic Access" 
                      : credits.subscriptionPlan === "plan_ZLLdnJssCPIpY" 
                        ? "Starter Plan" 
                        : "Analyses remaining"}
                  </p>
                </div>
                {credits.subscriptionPlan && (
                  <Badge variant="default" className="text-xs">
                    {credits.subscriptionPlan === "plan_mndBT74OUdiNB" ? "Basic" : "Starter"}
                  </Badge>
                )}
              </div>
              ...
              {onPurchase && (
                <Button
                  variant={credits.subscriptionPlan ? "outline" : "default"}
                  onClick={onPurchase}
                  disabled={isPurchasing}
                  className="w-full font-bold"
                  data-testid="button-get-unlimited-sidebar"
                >
                  {credits.subscriptionPlan ? "Upgrade Plan" : "Upgrade to Premium"}
                </Button>
              )}

              {credits.subscriptionPlan && subscriptionManageUrl && (
                <Button
                  variant="outline"
