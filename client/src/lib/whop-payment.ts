...
export async function purchaseCredits(planId: string, context?: { experienceId?: string; companyId?: string }): Promise<boolean> {
  // Send notification email when user clicks the button
  try {
    console.log("[Notification] Sending upgrade clicked notification...");
  ...
  try {
    console.log("[Payment] Creating checkout configuration for plan:", planId, "with context:", context);

    const configResponse = await fetch("/api/credits/checkout-config", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...(context || {}), planId }),
    });
    ...
    const { checkoutConfigId, planId: responsePlanId } = await configResponse.json();

    const actualPlanId = responsePlanId || planId;

    if (!checkoutConfigId || !actualPlanId) {
      console.error("Checkout configuration missing required data");
      return false;
    }

    console.log("[Payment] Opening payment modal with config:", { checkoutConfigId, planId: actualPlanId });
